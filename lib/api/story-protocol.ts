...
export class StoryProtocolClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      if (
        error instanceof StoryProtocolError &&
        error.retryable &&
        retries > 0
      ) {
  ...
  async getMultipleIPAssets(ipIds: string[]): Promise<IPAsset[]> {
    try {
      ...
      const requests = ipIds.map(ipId =>
        this.getIPAsset(ipId).catch(error => {
      ...
      const successfulResults = results
        .filter(
          (result): result is PromiseFulfilledResult<IPAsset | null> =>
  ...
  async getDerivativeRelations(
    ...
    try {
      ...
      this.logger(
        "error",
        `Error fetching derivative relations for ${ipId}:`,
        error
      )
      ...
      if (error instanceof StoryProtocolError) {
        if (
          error.type === StoryProtocolErrorType.NETWORK ||
  ...
  async getRelationshipsByType(
    ...
    try {
      ...
      this.logger(
        "error",
        `Error fetching ${type} relationships for ${ipId}:`,
        error
      )
  ...
  async getRelationship(
    ...
    try {
      ...
      this.logger(
        "error",
        `Error fetching relationship between ${sourceIpId} and ${targetIpId}:`,
        error
      )
  ...
  async getFullGraph(
    ...
    try {
      ...
      const queryParams = new URLSearchParams()
      if (options.maxDepth)
        queryParams.append("maxDepth", options.maxDepth.toString())
  ...
  async searchIPs(
    ...
    try {
      ...
      this.logger(
        "error",
        `Error searching for IPs with query "${query}":`,
        error
      )
 ...
 * @returns Derivative relationship data
 */
export const getDerivativeRelations = (
  ipId: string
): Promise<DerivativeRelationsResponse> => {
