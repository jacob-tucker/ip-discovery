...
const ForceGraph2D = dynamic(() => import('react-force-graph-2d'), {
  ...
  loading: () => (
    <div className="flex items-center justify-center h-full w-full">
      <div className="animate-pulse text-sm text-textMuted">Loading graph preview...</div>
    </div>
  )
});
  ...
  if (!graphData.nodes || graphData.nodes.length === 0) {
    return (
      <div
        ...
        style={{ position: 'absolute', width: '100%', height: '100%' }}
      >
        <div className="text-sm text-textMuted">No relationship data available</div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <ForceGraph2D
        ...
        nodeColor={(node: GraphNode) => getNodeColor(node, node.type === NodeType.ROOT)}
        ...
        onEngineStop={() => {
          ...
          if (graphData.nodes.length > 0) {
            graphData.nodes.forEach(node => {
              // @ts-ignore - Internal properties used by force-graph
              node.fx = node.x;
              // @ts-ignore
              node.fy = node.y;
            });
      ...
      <div className="absolute bottom-2 right-2 text-xs text-textMuted bg-background/80 px-2 py-1 rounded">
        {graphData.nodes.length} {graphData.nodes.length === 1 ? 'node' : 'nodes'}
      </div>
    </div>
  );
};
