...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      if (!apiKey || !chain) {
        console.error(
          "CRITICAL: X_API_KEY or X_CHAIN environment variables are not set in /api/ip/[ipId]/route.ts. This will cause API calls to fail."
        )
    ...
    let image = metadataJson.image ? transformIpfsUrl(metadataJson.image) : null
